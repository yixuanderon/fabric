...
func upgrade(cmd *cobra.Command, cf *ChaincodeCmdFactory) (*protcommon.Envelope, error) {
	...
	// delete old-versioned chaincode containers & images
	endpoint := "unix:///host/var/run/docker.sock"
	client, err := docker.NewClient(endpoint)
	if err != nil {
		return nil, fmt.Errorf("cannot connect to docker daemon when upgrade")
	}
	containers, err := client.ListContainers(docker.ListContainersOptions{All: true, Filters: map[string][]string{"name": {spec.ChaincodeId.Name}}})
	if err != nil {
		return nil, fmt.Errorf("cannot list containers to be deleted when upgrade")
	}
